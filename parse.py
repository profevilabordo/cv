...
input_text = """1. ¿Qué ciencia social estudia cómo las personas y sociedades toman decisiones ante necesidades ilimitadas y recursos limitados?
   a) Sociología
   b) Psicología
   c) Economía
   d) Historia
...
for line in lines:
    ...
    if m:
        ...
        q_num = int(m.group(1))
        if q_num == 1 and len(questions) >= 60:
            is_second_part = True
        ...
        current_q = {
            ...
            "c": 0,
            "q_num": q_num,
            "part": 2 if is_second_part else 1
        }
    elif re.match(r"^[a-d]\)", line):
        current_q["a"].append(line[3:].strip())
    elif "( ) Verdadero" in line:
        current_q["a"] = ["Verdadero", "Falso"]
    elif current_q and len(current_q["a"]) == 0:
        current_q["q"] += " " + line
...
for q in questions:
    part = q["part"]
    num = q["q_num"]
    ans = answers61_80[num] if part == 2 else answers1_60[num]
    
    if q["a"] == ["Verdadero", "Falso"]:
        c_idx = 0 if ans == "Verdadero" else 1
    else:
        c_idx = ord(ans) - 97
...
with open("economia_bank.json", "w", encoding="utf-8") as f:
    json.dump(final_q, f, ensure_ascii=False, indent=2)
